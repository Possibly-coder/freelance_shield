  ...
  return (
    <PageShell>
      <div className="text-center mb-16">
        <p className="text-sm font-semibold text-blue-400/80 tracking-widest uppercase mb-3">Project board</p>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="card flex items-start gap-4">
                <step.icon className="w-5 h-5 text-blue-400 flex-shrink-0 mt-1" />
                <div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="card flex items-start gap-4">
                <step.icon className="w-5 h-5 text-emerald-400 flex-shrink-0 mt-1" />
                <div>
                  <h3 className="font-semibold text-sm mb-1">{step.title}</h3>
                  <p className="text-gray-500 text-sm">{step.desc}</p>
                </div>
      ...
      <div className="card p-0 overflow-hidden mb-16">
        <div className="px-6 py-4 border-b border-[#1a2744] flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500">Preview -- live board available at launch</span>
        </div>
        {[
          ...
          <div key={i} className="px-6 py-4 border-b border-[#1a2744]/50 hover:bg-white/[0.01] transition-colors">
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-medium text-white">{project.title}</h4>
              <span className="text-sm text-gray-500">INR {project.budget}</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex gap-1.5">
                {project.skills.map((s) => (
                  <span key={s} className="text-xs bg-[#0a1128] border border-[#1a2744] text-gray-400 px-2 py-0.5 rounded-full">{s}</span>
                ))}
              </div>
      ...
      <div className="glass p-10 text-center glow-blue">
        ...
        <p className="text-gray-400 mb-6">Join the waitlist to get early access when we go live.</p>
